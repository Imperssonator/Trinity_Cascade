...
ternplot(A,C,B,'or')
ternlabel('Non-solvent','Polymer','Solvent')
toc
end
...
balls = 10000; %10000 balls per bag
...
while index ~= 1
    ...
    for i = 1:3
        if GRAD(1,i)>1 %if there are any balls of i in B1,
            GRAD(2*i,:) = Gibbs_calc(GRAD(1,1:3),GRAD(1,4:6),i,1); %find the energy of moving an A from bag 1
        end
        if GRAD(1,i+3)>1 %if there are any i in B2,
            GRAD(2*i+1,:) = Gibbs_calc(GRAD(1,1:3),GRAD(1,4:6),i,2); %find energy of moving A from bag 2
        end
    end
